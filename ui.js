window.UI = (() => {
  ...
  const UI_TEXT = {
    en: {
      ...
      'home.p.new': 'Turn your notes into a focused listening session.',
      ...
      'about.body': 'Dictator turns notes, essays, and study material into a deliberate listening practice. Everything runs in your browser, so your projects remain yours.',
    ...
    pt: {
      ...
      'home.p.new': 'Transforme suas anotações em uma sessão focada de escuta.',
      ...
      'about.body': 'O Dictator transforma notas, ensaios e material de estudo em uma prática de escuta deliberada. Tudo acontece no navegador, então seus projetos continuam sendo seus.',
...
Мира вдохнула один раз, потом ещё. «Хорошо», сказала она. «Давайте узнаем, что помнят часы». И спустилась.`
  ...
  function readerContent(project) { let lastType = ''; return project.groups.map(group => { const marker = group.hasTitle ? 'title' : group.hasSubtitle ? 'subtitle' : 'paragraph'; const heading = marker !== lastType && marker !== 'paragraph' ? `<${marker === 'title' ? 'h2' : 'h3'} class="reader-${marker}">${safe(group.rawText)}</${marker === 'title' ? 'h2' : 'h3'}>` : `<span class="word-group" data-index="${group.index}">${safe(group.rawText)}</span> `; lastType = marker; return heading; }).join(''); }
