(() => {
  ...
  function renderMenu() {
    ...
    document.querySelector('#page-menu').innerHTML='<div class="panel-header"><strong>' + t('nav.title', 'Navigate') + '</strong><button class="icon-button" data-close>×</button></div>' + nav.map(([path,label]) => `<button class="nav-link" data-route="${path}">${typeof label === 'string' && label.startsWith('nav.') ? t(label, label.replace('nav.','').replace(/^[a-z]/, c => c.toUpperCase())) : label}</button>`).join(''); document.querySelectorAll('[data-route]').forEach(button=>button.onclick=()=>{panel('#page-menu',false);Router.navigate(button.dataset.route);}); document.querySelector('[data-close]').onclick=()=>panel('#page-menu',false);
  }
